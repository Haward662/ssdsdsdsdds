...
import { Service, ReviewStory, Article } from './types';
...
// --- Navbar ---
const Navbar = ({ onOpenQuiz, isBlog, onHome }: { onOpenQuiz: () => void; isBlog: boolean; onHome: () => void }) => (
  <nav className="fixed top-0 w-full z-50 bg-black/40 backdrop-blur-2xl border-b border-white/5 px-4 md:px-8 py-4 md:py-6 flex justify-between items-center">
    <img src={LOGO_URL} alt="Logo" className="h-10 md:h-14 w-auto object-contain cursor-pointer" onClick={onHome} />
    <div className="hidden lg:flex space-x-12">
      {isBlog ? (
        <button onClick={onHome} className="text-[10px] uppercase tracking-[0.4em] font-black text-white/30 hover:text-indigo-400 transition-colors flex items-center gap-2">
    ...
    <button onClick={() => {
      trackEvent('cta_click', { text: 'Начать рост', location: 'navbar' });
      onOpenQuiz();
    }} className="bg-white text-black px-6 md:px-10 py-2.5 md:py-3 rounded-full font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-lg">Начать рост</button>
  </nav>
);
...
const FunnelStep = ({ step, index, smoothProgress }: { step: any, index: number, smoothProgress: any, key?: React.Key }) => {
  ...
  const baseRadius = windowWidth < 768 ? 400 : 1200;
  ...
  const textShadow = useTransform(glow, g => `0 0 ${g}px rgba(255,255,255,0.9)`);
  const subtitleOpacity = useTransform(stepScroll, [0.45, 0.5, 0.55], [0, 0.6, 0]);
...
// Rename useTransform to avoid confusion with the hook if needed, but let's just use it correctly.
// Actually, useTransform is the correct name. I'll use it directly.
    ...
    <section ref={sectionRef} className="relative h-[800vh] bg-black">
      <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative w-full h-full flex items-center justify-center z-20">
          {steps.map((step, i) => (
            <FunnelStep key={i} step={step} index={i} smoothProgress={smoothProgress} />
          ))}
        </div>
      </div>
    </section>
        ...
        <div ref={scrollRef} className="flex gap-6 md:gap-10 overflow-x-auto no-scrollbar pb-10 md:pb-20 px-4 snap-x">
          {REVIEWS.map(r => (
            <motion.div 
              ...
              className="flex-shrink-0 w-[280px] md:w-[350px] h-[498px] md:h-[622px] rounded-[2.5rem] md:rounded-[4rem] overflow-hidden relative cursor-pointer border border-white/10 group shadow-2xl bg-neutral-900 snap-center"
            >
...
// --- Blog Article Page ---
const ArticlePage = ({ article, onBack, trackEvent }: { article: Article; onBack: () => void; trackEvent: (name: string, data?: any) => void }) => {
  useEffect(() => { window.scrollTo(0, 0); }, []);
  return (
    <div className="min-h-screen bg-black text-white pt-28 pb-24 px-6 md:px-8">
      <div className="max-w-[860px] mx-auto">
        <button onClick={onBack} className="flex items-center gap-3 text-white/40 hover:text-indigo-400 transition-colors mb-12 text-sm font-black uppercase tracking-widest">
        ...
        <div className="mb-6 flex flex-wrap gap-3 items-center">
          <span className="px-4 py-1.5 rounded-full bg-indigo-600/20 border border-indigo-500/30 text-indigo-400 text-xs font-black uppercase tracking-widest">{article.category}</span>
          <span className="text-white/20 text-xs font-bold uppercase tracking-widest">{article.readTime} мин читать</span>
          <span className="text-white/20 text-xs font-bold uppercase tracking-widest">{new Date(article.publishedAt).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
        </div>
        ...
        <div className="mt-16 pt-8 border-t border-white/10">
          <div className="flex flex-wrap gap-2 mb-12">
            {article.tags.map(tag => (
              <span key={tag} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-white/40 text-xs font-bold"># {tag}</span>
            ))}
          </div>
          <div className="glass-card p-8 md:p-12 rounded-[2rem] border border-indigo-500/20 text-center">
            <h3 className="text-2xl md:text-4xl font-black text-white italic uppercase mb-4">Нужна помощь с маркетингом?</h3>
            <p className="text-white/40 mb-8 uppercase font-bold text-sm tracking-widest">Обсудим ваш проект и составим стратегию роста</p>
            <a href="#contact" onClick={onBack} className="inline-block px-10 py-5 bg-white text-black font-black text-base rounded-full hover:bg-indigo-600 hover:text-white transition-all uppercase italic tracking-tighter">Обсудить проект</a>
          </div>
        </div>
      </div>
    </div>
...
const BlogPage = ({ articles, onArticle, trackEvent }: { articles: Article[]; onArticle: (slug: string) => void; trackEvent: (name: string, data?: any) => void }) => {
  ...
  const categories = ['Все', ...Array.from(new Set(sorted.map(a => a.category)))];
  const filtered = filter === 'Все' ? sorted : sorted.filter(a => a.category === filter);

  return (
    <div className="min-h-screen bg-black text-white pt-28 pb-24 px-6 md:px-8">
      <div className="max-w-[1400px] mx-auto">
        <div className="mb-16 md:mb-24">
          <h1 className="text-5xl md:text-[9rem] font-black text-white italic uppercase leading-none tracking-tighter mb-6">БЛОГ<br/><span className="text-indigo-600">PROBOOST</span></h1>
          <p className="text-white/30 text-lg md:text-2xl font-bold uppercase italic tracking-widest max-w-2xl">Маркетинг для доставок еды и не только — кейсы, инструменты, стратегии</p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          {categories.map(cat => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all border ${filter === cat ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-transparent border-white/10 text-white/40 hover:border-indigo-500/50 hover:text-indigo-400'}`}
            >
              {cat}
            </button>
          ))}
            ...
            >
              ...
              <div className="p-6 md:p-8">
                <div className="flex items-center gap-3 mb-4">
                  <span className="px-3 py-1 rounded-full bg-indigo-600/20 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-widest">{article.category}</span>
                  <span className="text-white/20 text-[10px] font-bold uppercase">{article.readTime} мин</span>
                </div>
                <h2 className="text-lg md:text-xl font-black text-white italic uppercase leading-tight tracking-tighter mb-3 group-hover:text-indigo-400 transition-colors">{article.title}</h2>
                <p className="text-white/40 text-sm leading-relaxed mb-6 line-clamp-3">{article.excerpt}</p>
                <div className="flex items-center justify-between">
                  <span className="text-white/20 text-xs font-bold uppercase">{new Date(article.publishedAt).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                  <span className="text-indigo-400 text-xs font-black uppercase tracking-widest flex items-center gap-2 group-hover:gap-3 transition-all">
                    Читать
...
const App = () => {
  ...
  const USER_ID = useMemo(() => {
    let id = localStorage.getItem('analyticsUserId');
    if (!id) {
      id = `user-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
      localStorage.setItem('analyticsUserId', id);
    }
    return id;
  }, []);
  ...
  const trackEvent = useCallback((eventName: string, data?: any) => {
    try {
      fetch(`${ANALYTICS_URL}/api/analytics`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          event: eventName,
  ...
  useEffect(() => {
    ...
    const handleUnload = () => {
      ...
      trackEvent('page_unload', { timeSpent });
    };
  ...
  const goTo = useCallback((h: string) => {
    window.location.hash = h;
    setHash(h);
  }, []);

  // hash routing: #/blog or #/blog/slug → blog pages; everything else → main SPA
  const isBlogRoot = hash === '#/blog';
  const blogSlug = hash.startsWith('#/blog/') ? hash.replace('#/blog/', '') : null;
  const currentArticle = blogSlug ? articles.find(a => a.slug === blogSlug) : null;
  const isBlog = isBlogRoot || !!blogSlug;

  // Обновляем метатеги для статьи при открытии
  useEffect(() => {
    if (currentArticle) {
      updateMetaTags(
        ...
        `https://pro-boost.ru/#/blog/${currentArticle.slug}`
      );
    } else if (hash === '' || hash === '#/') {
      // Главная страница
      updateMetaTags(
        'PROBOOST | Маркетинг для доставки еды и ресторанов',
        'Увеличиваем заказы для ресторанов и доставок еды. Таргет ВКонтакте, Яндекс Директ, геомаркетинг. Окупаемость с первого месяца.',
        'https://i.ibb.co/0pzdjPSh/Chat-GPT-Image-22-2025-12-19-19.png',
        'https://pro-boost.ru/'
      );
    } else if (isBlogRoot) {
      // Страница блога
      updateMetaTags(
        'Блог PROBOOST | Маркетинг для доставки еды',
        'Кейсы, стратегии и инструменты маркетинга для ресторанов и доставок еды.',
        'https://i.ibb.co/0pzdjPSh/Chat-GPT-Image-22-2025-12-19-19.png',
        'https://pro-boost.ru/#/blog'
      );
    }
  }, [currentArticle, hash, isBlogRoot]);
    ...
    <footer className="py-16 md:py-24 px-6 md:px-8 border-t border-white/5 bg-black">
      <div className="max-w-[1500px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-10 md:gap-16">
        <div className="space-y-6 md:space-y-8">
          <img src={LOGO_URL} alt="Logo" className="h-12 md:h-20 opacity-30 grayscale hover:opacity-100 transition-all cursor-pointer" onClick={() => goTo('')} />
        ...
        <div className="flex flex-col items-start md:items-end gap-4">
          <a href="#/blog" onClick={() => goTo('#/blog')} className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 hover:text-indigo-400 transition-colors border border-white/10 hover:border-indigo-500/40 px-4 py-2 rounded-full">
            Блог
          </a>
    ...
    <div className="bg-black text-white min-h-screen selection:bg-indigo-500 antialiased font-['Inter']">
      ...
      {!isLoading && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
          <CustomCursor />
          <Navbar onOpenQuiz={() => setIsQuiz(true)} isBlog={isBlog} onHome={() => goTo('')} />
